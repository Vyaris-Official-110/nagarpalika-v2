...
export const mongoSanitizer = mongoSanitize({
  ...
  onSanitize: ({ req, key }) => {
    console.warn(
      `[SECURITY] Sanitized potentially malicious input in key: ${key}`,
    );
  },
});
...
export const emailValidator = body("email")
  ...
  .isLength({ max: MAX_LENGTHS.EMAIL })
  .withMessage(`Email must not exceed ${MAX_LENGTHS.EMAIL} characters`)
  .normalizeEmail();
...
export const passwordValidator = body("password")
  ...
  .withMessage(
    `Password must be between ${MIN_LENGTHS.PASSWORD} and ${MAX_LENGTHS.PASSWORD} characters`,
  );
...
export const nameValidator = (fieldName) =>
  body(fieldName)
    ...
    .withMessage(
      `${fieldName} must be between ${MIN_LENGTHS.NAME} and ${MAX_LENGTHS.NAME} characters`,
    )
 ...
 */
export const optionalNameValidator = (fieldName) =>
  body(fieldName)
    .optional()
    .trim()
 ...
 */
export const phoneValidator = (fieldName = "mobileNumber") =>
  body(fieldName)
    .optional()
    .trim()
...
export const paginationValidators = [
  ...
  body("sorton")
    ...
    .trim()
    .isLength({ max: 50 })
    .withMessage("sorton must not exceed 50 characters")
    .matches(/^[a-zA-Z_]+$/)
    .withMessage("sorton contains invalid characters"),
  body("sortdir")
    .optional()
    .trim()
  ...
  body("match")
    ...
    .withMessage(`match must not exceed ${MAX_LENGTHS.SHORT_TEXT} characters`)
    .customSanitizer(sanitizeString),
];
...
export const createEmployeeValidation = [
  ...
  body("address")
    ...
    .withMessage(
      `Address must not exceed ${MAX_LENGTHS.MEDIUM_TEXT} characters`,
    )
    .customSanitizer(sanitizeString),
...
export const allowedLoginFields = ["email", "password", "totpToken"];
...
const requiredPhoneValidator = (fieldName) =>
  body(fieldName)
    .trim()
    .notEmpty()
    .withMessage(`${fieldName} is required`)
...
const optionalMongoIdBody = (fieldName) =>
  body(fieldName)
...
const optionalLongText = (fieldName, max = MAX_LENGTHS.LONG_TEXT) =>
  body(fieldName)
